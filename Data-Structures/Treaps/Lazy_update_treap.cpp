...
void push(tnode *t) {
    if (t && t->rev) {
        t->rev = 0;           // Clear the flag
        swap(t->c[0], t->c[1]);   // Swap the children
        
        // Pass the lazy flag down to the children
...
int main(){
    ...
    while(q--){
        ...
        if (b.first) b.first->rev ^= 1;
        
        t = merge(merge(a.first, b.first), b.second);
    }
    print(t);
    cout << endl;
}
